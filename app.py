...
def grade_quiz(q1, q2, q3, q4, q5, quiz):
    if not quiz:
    ...
    for i, (user_answer, q) in enumerate(zip(answers, quiz["questions"])):
        ...
        correct = grade_answer(correct, feedback, i, q, user_answer)
    score_text = f"Your score: {correct} / {len(quiz['questions'])}"
    feedback_text = "\n".join(feedback)
    return f"{score_text}\n\n{feedback_text}"


def grade_answer(correct, feedback, i, q, user_answer):
    try:
        ...
        if user_answer.endswith(f"{correct_answer}"):
            ...
            for j, option in enumerate(q["options"]):
                if option == correct_answer:
                    ...
                    break
            feedback.append(
                f"Q{i + 1}: ❌ Incorrect. You chose {user_answer}, correct answer is {correct_letter}. {correct_answer}")
    except:
        feedback.append(f"Q{i + 1}: ❌ Error processing answer")
    return correct


def populate_quiz_ui(quiz_json):
    try:
        quiz = json.loads(quiz_json)
    except:
        return quiz_json, gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(
            visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), None
    if not quiz.get("questions"):
        return quiz_json, gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(
            visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), None
    updates = populate_questions_and_answers(quiz, quiz_json)
...
def populate_questions_and_answers(quiz, quiz_json):
...
with gr.Blocks() as demo:
    ...
    quiz_json_box = gr.Textbox(label="Raw Quiz JSON (placeholder, running takes about 30s since I don't stream the tokens as they're generated)", visible=True)
    ...
    def handle_generate(prompt):
        ...
        return populate_quiz_ui(quiz_json)

    def handle_submit(q1, q2, q3, q4, q5, quiz):
        return grade_quiz(q1, q2, q3, q4, q5, quiz)
